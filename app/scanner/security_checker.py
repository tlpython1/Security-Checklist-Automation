...
from scanner.port_scanner import scan_ports, get_open_ports_with_remote_ips, get_comprehensive_port_scan
...
def run_full_scan(host, port, username, password=None, ssh_key_path=None, key_passphrase=None, comprehensive_scan=False):
    try:
        ...
        with Connection(
            ...
            if comprehensive_scan:
                logger.info("Running comprehensive port scan (all 65535 ports)...")
                open_ports = get_comprehensive_port_scan(host)
            else:
                logger.info("Running standard port scan (top 1000 ports)...")
                open_ports = get_open_ports_with_remote_ips(host)
            
            scan_results['open_ports'] = open_ports
            logger.info(f"Open ports found: {len(open_ports)} ports")
            ...
            try:
                sensitive_files = check_sensitive_files(conn)
                scan_results['sensitive_files'] = sensitive_files
                logger.info(f"Sensitive files found: {len(sensitive_files)}")
            except Exception as e:
            ...
            scan_results['security_summary'] = {
                ...
                'high_risk_ports': [port for port in open_ports.keys() if port in [22, 23, 80, 443, 3389, 5900]],
                'sensitive_files_count': len(scan_results['sensitive_files']),
                'docker_issues_count': len(scan_results['docker_issues']),
        ...
        logger.error(f"Failed to connect or scan: {e}")
        return {
